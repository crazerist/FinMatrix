...
Section vzero.
  ...
  Definition vzero {n : nat} : @vec tA n := vrepeat Azero.

  (** vzero.i = 0 *)
  Lemma vnth_vzero : forall {n} i, (@vzero n).[i] = Azero.
  Proof. intros. apply vnth_vrepeat. Qed.

  Lemma vneq_vzero : forall {n} (v : @vec tA n),
    v <> vzero <-> exists i : 'I_n, v i <>  Azero.
  Proof.
  split; intros.
  - apply not_all_not_ex; intro. apply H. extensionality x.
    specialize H0 with x. apply NNPP in H0. rewrite vnth_vzero; auto.
  - destruct H as [i H]. intro. apply H. rewrite  H0. rewrite vnth_vzero; auto.
  Qed.
...
Section l2v_v2l.
  ...
  Proof. intros. unfold v2l. rewrite length_map, finlength_seq. auto. Qed.
...
Section veye.
  ...
  Lemma veye_eq : forall {n} i j, (@veye n i) = (@veye n j) <-> i = j.
  Proof.
    intros. split; intros. 2:{ subst; auto. } 
    apply NNPP. intro. assert (veye i i = veye j i) by (rewrite H; auto).
    unfold veye in H1. destruct (i ??= i); destruct (j ??= i); fin.
  Qed.
...
Section vscale.
  ...
  Definition vscale {n : nat} (a : vec n) (i : 'I_n) (x : tA) : vec n := 
    fun j : 'I_n => if (j ??= i) then x * a.[i] else a.[j].
...
Section vadde.
  ...
  Definition vadde {n : nat} (a : vec n) (i j: 'I_n) (x : tA) : vec n :=
    fun k : 'I_n => if (k ??= i) then a.[i] + (x * a.[j]) else a.[k].
End vadde.
...
Section vremove.
  ...
  Lemma vinsert_vremove_eq : forall {n} (a : @vec tA (S n)) (i : 'I_(S n)) (val : tA),
    vinsert (vremove a i) i val = vset a i val.
  Proof.
    intros. extensionality b. destruct (b ??< i) as [E|E]; [|destruct (i ??< b) as [E'|E']].
    - erewrite vnth_vinsert_lt. erewrite vnth_vremove_lt; fin. simpl.
      unfold vset. destruct (b??=i); fin. erewrite nth_v2f; fin.
      Unshelve. exact (Azero). apply E. pose proof (fin2nat_lt b); auto.
    - assert (fin2nat b <> 0)%nat. { intro; rewrite H in E'. lia. }
      assert (0 < fin2nat b)%nat by lia. erewrite vnth_vinsert_gt; fin.
      instantiate (1:=H0). rewrite vnth_vremove_ge; fin. 2: { pose proof (fin2nat_lt b). fin. }
      unfold vset. destruct (b??=i); fin. assert (S (pred b) < S n). { pose proof (fin2nat_lt b). fin. }
      erewrite nth_v2f; fin.  instantiate (1:=H1). apply fin2nat_eq_iff. rewrite fin2nat_nat2fin. lia.
      Unshelve. exact (Azero).
    - assert (fin2nat b = i). { lia. } rewrite fin2nat_eq_iff in H; subst.
      unfold vset. destruct (i ??= i); fin.
      rewrite (@vnth_vinsert_eq _ Azero). auto.
  Qed.
...
Section vremoveH_vremoveT.
  ...
  Lemma remove_removeT : forall {n} (a : @vec tA (S n)),
    vremove a #n =  vremoveT a.
  Proof.
    intros. unfold vremove. unfold vremoveT. extensionality x.
    destruct (x ??< #n); auto. pose proof (fin2nat_lt x).
    rewrite fin2nat_nat2finS in n0. lia. lia.
  Qed.
...
(* ======================================================================= *)
(** ** vpivot, calculate the first non-zero element *)
Section vpivot.
  Context {tA} (Azero : tA).
  ...
  Notation "0" := Azero : A_scope.
   ...
   (** Use a recursive method similar to listFirstNonZero to calculate the first non-zero element*)
  Fixpoint vpivot {n} : @vec tA n -> option ('I_n) :=
   match n with
    | 0 => fun _ => None
    | S n' => fun v : vec (S n')=>
      if Aeqdec (vhead v) 0 
      then  match vpivot (vremoveH v) with
        | Some j => Some (fSuccRangeS j)
        | None => None
        end
      else Some fin0
    end.

  (** When the length of vector is 0, then vpivot always get None *)
  Lemma vpivot_0 : forall (v : vec 0),
    vpivot v = None.
  Proof. intros. cbv; auto. Qed.

  (** When a element is added to the first element of the original vector *)
  Lemma vpivot_vconsH : forall {n} (v : vec n) (a : tA),
    vpivot (vconsH a v) = 
    if Aeqdec a 0
    then match vpivot v with 
      | Some i => Some (fSuccRangeS i) 
      | None => None
      end
    else Some fin0.
  Proof. 
    intros. simpl vpivot. rewrite vhead_vconsH.
    destruct (Aeqdec a 0).
    - (* a = 0 *)
      rewrite vremoveH_vconsH. auto.
    - (* a <> 0 *)
      auto.
  Qed. 
...
Section vdot.
  ...
  Lemma vdot_0 : forall (a b : vec 0), <a, b> = Azero.
  Proof.
    intros. unfold vdot. fin.
  Qed.
  ...
  Section ARing.
    ...
    Lemma vdot_vinsert_l : forall n i (a : vec n) (b : vec (S n)) val,
    <vinsert a i val, b > = (<a, vremove b i> + val * b.[i])%A.
    Proof.
    intros. unfold vdot. rewrite (vmap2_vinsert_l (Azero:=Azero)(Bzero:=Azero)(Czero:=Azero)).
    erewrite vsum_vinsert. auto. Unshelve. all : fin.
    Qed.

    Lemma vdot_vinsert_0_l : forall n i (a : vec n) (b : vec (S n)),
    <vinsert a i 0, b > = (<a, vremove b i> )%A.
    Proof.
    intros. unfold vdot. rewrite (vmap2_vinsert_l (Azero:=Azero)(Bzero:=Azero)(Czero:=Azero)).
    erewrite vsum_vinsert. auto. Unshelve. all : fin.
    Qed.

    Lemma vdot_vinsert_r : forall n i (a : vec (S n)) (b : vec n) val,
    <a, vinsert b i val > = (<vremove a i, b> + val * a.[i])%A.
    Proof.
    intros. unfold vdot. rewrite (vmap2_vinsert_r (Azero:=Azero)(Bzero:=Azero)(Czero:=Azero)).
    erewrite vsum_vinsert. auto. Unshelve. all : fin.
    Qed.

    Lemma vdot_vinsert_0_r : forall n i (a : vec (S n)) (b : vec n),
    <a, vinsert b i 0 > = (<vremove a i, b>)%A.
    Proof.
    intros. unfold vdot. rewrite (vmap2_vinsert_r (Azero:=Azero)(Bzero:=Azero)(Czero:=Azero)).
    erewrite vsum_vinsert. auto. Unshelve. all : fin.
    Qed.

   Lemma vdot_vinsert : forall n i (a : vec n) (b : vec n) val1 val2,
      <vinsert a i val1, vinsert b i val2 > = 
      (<a, b> + val1 * val2)%A.
    Proof.
    intros. rewrite vdot_vinsert_l. erewrite vremove_vinsert.
    erewrite vnth_vinsert_eq. auto. Unshelve. all : fin.
    Qed.


      Lemma vdot_vremove : forall n i (a : vec (S n)) (b : vec (S n)),
      <vremove a i, vremove b i > = 
      (<a, b> - a.[i] * b.[i])%A.
    Proof.
    intros. replace a with (vinsert (vremove a i) i a.[i]) at 2.
    replace b with (vinsert (vremove b i) i b.[i]) at 2.
    rewrite vdot_vinsert. rewrite associative. fin.
    erewrite vinsert_vremove; auto.
    erewrite vinsert_vremove; auto. Unshelve. all : fin.
    Qed.


     Lemma vdot_vset_l : forall n i (a : vec (S n)) (b : vec (S n)) val,
      <vset a i val, b> = (<vremove a i, vremove b i> + val * b.[i])%A.
    Proof.
      intros. erewrite <- vinsert_vremove_eq. rewrite vdot_vinsert_l.
      auto. Unshelve. fin.
    Qed. 

    Lemma vdot_vset_r : forall n i (a : vec (S n)) (b : vec (S n)) val,
      <a, vset b i val> = (<vremove a i, vremove b i> + val * a.[i])%A.
    Proof.
      intros. erewrite <- vinsert_vremove_eq. rewrite vdot_vinsert_r.
      auto. Unshelve. fin.
    Qed.

    Lemma vdot_vset : forall n i (a : vec (S n)) (b : vec (S n)) val1 val2,
      <vset a i val1, vset b i val2> = (<a, b> - a.[i] * b.[i] + val1 * val2)%A.
    Proof.
      intros. erewrite vdot_vset_l. rewrite vdot_vremove. rewrite vdot_vset_r.
      rewrite vdot_vremove. unfold vset. destruct (i ??= i); fin.
    Qed.

    Lemma vdot_vswap : forall n  (a : vec (S n)) (b : vec (S n)) i j,
      <vswap a i j, vswap b i j> = <a, b>.
    Proof.
      intros. replace (vswap a i j) with (vset (vset a i a.[j]) j a.[i]).
      replace (vswap b i j) with (vset (vset b i b.[j]) j b.[i]).
      rewrite vdot_vset. replace (vset a i (a j) j) with (a j).
      replace (vset b i (b j) j) with (b j). rewrite vdot_vset. fin.
      - unfold vset. destruct (j??=i); fin. 
      - unfold vset. destruct (j??=i); fin. 
      - extensionality p. unfold vset, vswap.
        destruct (p ??= j); destruct (p ??= i); fin.
        apply fin2nat_eq_iff. rewrite <- e. rewrite <- e0. auto.
      - extensionality p. unfold vset, vswap.
        destruct (p ??= j); destruct (p ??= i); fin.
        apply fin2nat_eq_iff. rewrite <- e. rewrite <- e0. auto.
    Qed.
...
Section vsum_rowtrans.
  ...
  Lemma vsum_vswap :
    forall {n s} (v : @vec (@vec tA s) n) (i j : 'I_s),
    vswap (vsum v) i j = vsum (vmap (fun v' => vswap v' i j) v).
  Proof.
    intros. rewrite veq_iff_vnth. intros a. rewrite vnth_vsum.
    replace (fun i0 : 'I_n => vmap (fun v' : vec s => vswap v' i j) v i0 a) with 
      (fun i0 : 'I_n => vswap (v i0) i j a).
    2:{ extensionality b. rewrite vnth_vmap. auto. }
    unfold vswap. destruct (a ??= i) as [E|E]; destruct (a ??= j) as [E'|E'];
    rewrite vnth_vsum; fin.
  Qed.

  Lemma vsum_vscale :
    forall {n s} (v : @vec (@vec tA s) n) (i : 'I_s) (k : tA),
    vscale (vsum v) i k  = vsum (vmap (fun v' => vscale v' i k) v).
  Proof.
    intros. rewrite veq_iff_vnth. intros a. rewrite vnth_vsum.
    replace (fun i0 : 'I_n => vmap (fun v' : vec s => vscale v' i k) v i0 a) 
      with (fun i0 : 'I_n => vscale (v i0) i k a).
    2:{ extensionality b. rewrite vnth_vmap. auto. }
    unfold vscale. destruct (a ??= i) as [E|E]; rewrite vnth_vsum; fin.
    apply vsum_scal_l_ext; fin.
  Qed.

  Lemma vsum_vadde :
    forall {n s} (v : @vec (@vec tA s) n) (i j: 'I_s) (k : tA),
    vadde (vsum v) i j k = vsum (vmap (fun v' => vadde v' i j k) v).
  Proof.
    intros. rewrite veq_iff_vnth. intros a. rewrite vnth_vsum.
    replace ((fun i0 : 'I_n => vmap (fun v' : vec s => vadde v' i j k) v i0 a)) 
      with (fun i0 : 'I_n => vadde (v i0) i j k a).
    2:{ extensionality b. rewrite vnth_vmap. auto. }
    unfold vadde. destruct (a ??= i) as [E|E]; rewrite vnth_vsum; fin.
    rewrite vnth_vsum. rewrite vsum_scal_l. rewrite vsum_add. auto.
  Qed.
...
Section vascend.
  ...
  Notation vopp := (@vopp _ Aopp).
  Notation vdot := (@vdot _ Aadd Azero Amul).
  Variable (Alt : tA -> tA -> Prop).
  
  (** Any element in the ascending vector is smaller than the next element *)
  Definition vascend {n} (v : @vec tA (S n)) : Prop :=
    forall i : 'I_n, Alt v.[fSuccRange i] v.[fSuccRangeS i].

  (** A vector with only one element is always ascending *)
  Lemma vascend_1 : forall (v : @vec tA 1),
    vascend v.
  Proof. unfold vascend; intros. fin. Qed.
    
  (** A vector is a ascending vector then remove tail element is also ascending *)
  Lemma vascend_vremoveT : forall {n} (v : @vec tA (S (S n))),
    vascend v -> vascend (vremoveT v).
  Proof. 
    unfold vascend; intros. specialize H with (fSuccRange i).
    replace (vremoveT v (fSuccRange i)) with (v (fSuccRange (fSuccRange i))).
    replace (vremoveT v (fSuccRangeS i)) with (v (fSuccRangeS (fSuccRange i))). auto.
    rewrite vnth_vremoveT. f_equal. apply nat2fin_iff_fin2nat. fin.
    rewrite vnth_vremoveT. f_equal.
  Qed.
