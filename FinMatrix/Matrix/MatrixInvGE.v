...
Section minvGE.
  ...
  Definition minvtblebGE {n} : smat n -> bool :=
    ...
    | S n' =>
        fun M =>
          match sm2REF M (S n') with
          | None => false
          | Some (l1, M1) => true
          end
    end.

  (* M * N = mat1 -> (exists (l1, M1), sm2REF M (S n) = Some (l1,M1) *)
  Lemma mmul_eq1_imply_sm2REF_Some_l : forall {n} (M N : smat (S n)),
      M * N = mat1 -> (exists '(l1, M1), sm2REF M (S n) = Some (l1, M1)).
  Proof.
  Admitted.

  (* M * N = mat1 -> (exists (l1, N1), sm2REF N (S n) = Some (l1,N1) *)
  Lemma mmul_eq1_imply_sm2REF_Some_r : forall {n} (M N : smat (S n)),
      M * N = mat1 -> (exists '(l1, N1), sm2REF N (S n) = Some (l1, N1)).
  Proof.
  Admitted.
  ...
  Proof.
    ...
    - hnf in H. destruct H as [M' [Hl Hr]]. destruct n; auto.
      apply (mmul_eq1_imply_sm2REF_Some_r) in Hl. destruct Hl as [[l1 M1]].
      unfold minvtblebGE. rewrite H. auto.
    - apply minvtble_iff_minvtbleL. hnf.
      unfold minvtblebGE in H. destruct n.
      + exists M. apply v0eq.
      + destruct sm2REF as [[l1 M1]|] eqn:T1; try easy.
        destruct (sm2RREF M1 (S n)) as [l2 M2] eqn:T2.
        apply sm2RREF_eq in T2 as H3.
        apply sm2REF_eq in T1 as H4.
        apply sm2RREF_mat1 in T2 as H5.
        * subst. rewrite <- mmul_assoc in H5.
          exists (rowOps2mat l2 * rowOps2mat l1); auto.
        * apply sm2REF_mUnitUpperTrig in T1. auto.
  Qed.
  ...
  Proof.
    ...
    - split; intros.
      + destruct H as [M' H]. destruct sm2REF as [[l1 M1]|]; try easy.
      + destruct sm2REF as [[l1 M1]|] eqn:T1; try easy.
        destruct sm2RREF as [l2 M2] eqn:T2. eexists; auto.
  Qed.
  ...
  Proof.
    ...
    - split; intros; try easy.
      ...
      + destruct sm2REF as [[l1 M1]|] eqn:T1; try easy.
        destruct sm2RREF as [l2 M2] eqn:T2. destruct H. eexists; auto.
  Qed.
  ...
  Proof.
    ...
    - destruct sm2REF as [[l1 M1]|] eqn:T1; try easy.
      ...
      copy T1. copy T2.
      apply sm2REF_eq in T1.
      apply sm2RREF_eq in T2.
      rewrite rowOps2mat_app. rewrite mmul_assoc. rewrite T1,T2.
      apply sm2RREF_mat1 in HC0; auto.
      apply sm2REF_mUnitUpperTrig in HC; auto.
  Qed.
  ...
  Proof.
    ...
    - destruct sm2REF as [[l1 M1]|] eqn:T1; try easy.
      ...
      apply sm2REF_eq_inv in T1 as T1'.
      apply sm2RREF_eq_inv in T2 as T2'.
      apply sm2RREF_mat1 in T2 as T2''; auto.
      2:{ apply sm2REF_mUnitUpperTrig in T1; auto. }
      ...
      apply sm2RREF_rowOpValid in T2; auto.
      apply sm2REF_rowOpValid in T1; auto.
  Qed.
  ...
  Definition minvGE {n} : smat n -> smat n :=
    ...
    | S n' =>
        fun (M : smat (S n')) =>
          match sm2REF M n with
          | None => mat1
          | Some (l1, M1) =>
              let (l2, M2) := sm2RREF M1 n in
              rowOps2mat (l2 ++ l1)
          end
    end.
  ...
  Proof.
    ...
    apply sm2REF_eq in T1 as T1'. rewrite T1'.
    apply sm2RREF_eq in T2 as T2'. rewrite T2'.
    apply sm2RREF_mat1 in T2; auto.
    apply sm2REF_mUnitUpperTrig in T1; auto.
  Qed.
