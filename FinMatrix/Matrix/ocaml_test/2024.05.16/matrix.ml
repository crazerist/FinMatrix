...
let rec getcolPivot azero hAeqDec n m x j =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun x' ->
    if aeqdec hAeqDec (m (nat2finS n (sub (Stdlib.Int.succ n) x)) j) azero
    then getcolPivot azero hAeqDec n m x' j
...
let rec rowisREF aadd azero aopp amul ainv hAeqDec n m x =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> Some ([], m))
    (fun x' ->
    let j = nat2finS n (sub (Stdlib.Int.succ n) x) in
    (match getcolPivot azero hAeqDec n m x j with
     | Some i ->
       ...
       then let op1 = ROnop in
            ...
            in
            let (l3, m3) = smElimDown aadd azero aopp amul hAeqDec n m2 x' j in
            (match rowisREF aadd azero aopp amul ainv hAeqDec n m3 x' with
             | Some p ->
...
let rec minRowisREF aadd azero aopp amul hAeqDec n m x =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> ([], m))
    (fun x' ->
    let fx = nat2finS n x' in
    let (l1, m1) = smElimUp aadd azero aopp amul hAeqDec n m x' fx in
    let (l2, m2) = minRowisREF aadd azero aopp amul hAeqDec n m1 x' in ((app l2 l1), m2))
    x
...
let minvtbleb0 aadd azero aopp amul ainv aeqDec n x =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> true)
    (fun n' ->
    match rowisREF aadd azero aopp amul ainv aeqDec n' x (Stdlib.Int.succ n') with
    | Some _ -> true
    | None -> false)
    n
...
let minvo0 aadd azero aopp amul aone ainv aeqDec n m =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    (fun n' ->
    match rowisREF aadd azero aopp amul ainv aeqDec n' m (Stdlib.Int.succ n') with
    | Some p ->
      let (l1, m1) = p in
      let (l2, _) = minRowisREF aadd azero aopp amul aeqDec n' m1 (Stdlib.Int.succ n') in
...
let minv0 aadd azero aopp amul aone ainv aeqDec n x =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    (fun n' ->
    match rowisREF aadd azero aopp amul ainv aeqDec n' x n with
    | Some p ->
      let (l1, m1) = p in
      let (l2, _) = minRowisREF aadd azero aopp amul aeqDec n' m1 n in
