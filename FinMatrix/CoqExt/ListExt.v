...
Section concat.
  ...
  Proof.
    ...
    rewrite length_app. rewrite IHl. rewrite (fold_left_nat_initial _ (length a)).
    lia.
  Qed.
...
Section dmap_A_B.
  ...
  (** dmap length law *)
  Lemma dlength_map : forall dl, length (dmap dl) = length dl.
  Proof.
    induction dl; simpl; auto.
  Qed.
...
Section dlremove.
  ...
  Proof.
    ...
    constructor; auto. rewrite length_firstn. lia. apply IHdl with (c:=c); auto.
  Qed.
  ...
  Proof.
    induction dl; intros; simpl in *; auto. constructor. inversion H.
    constructor; auto. rewrite length_skipn. lia. apply IHdl with (c:=c); auto.
  Qed.
