...
Section ss_eq.
  ...
  Definition ss_eq : Prop :=
    forall v : V, Hbelong ss v <-> Hbelong ss' v.

End ss_eq.

Section ss_eq_theory.
  Context `(ss : SubSpaceStruct).
  Context `{P' : V -> Prop}.
  Context `(ss' : @SubSpaceStruct tA Aadd Azero Aopp Amul Aone Ainv F V Vadd Vzero Vopp Vscal HVectorSpace P').
  Context `{P'' : V -> Prop}.
  ...
  Lemma ss_eq_refl : ss_eq ss ss.
Proof. unfold ss_eq. intros. split; auto. Qed.

  Lemma ss_eq_sym : ss_eq ss ss' -> ss_eq ss' ss.
Proof. unfold ss_eq. intros. split; intros. rewrite H; auto. rewrite <- H; auto. Qed.

  Lemma ss_eq_trans : ss_eq ss ss' -> ss_eq ss' ss'' -> ss_eq ss ss''.
  Proof. 
    unfold ss_eq. intros. split; intros. rewrite <- H0. rewrite <- H.
    auto. rewrite H. rewrite H0. auto.
  Qed.
