  ...
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    (fun fO fS n -> if n=0 then fO () else fS (n-1))
      ...
      then isRREFSolve azero aopp aone r c m b v x' y' ((fst ans), (fun i ->
             if nat_eq_Dec (fin2nat (Int.succ c) i) (fin2nat (Int.succ c) a)
             then b (nat2finS r x')
             else snd ans i))
...
let toRREF'_R =
  toRREF' RbaseSymbolsImpl.coq_Rplus RbaseSymbolsImpl.coq_R0
    RbaseSymbolsImpl.coq_Ropp RbaseSymbolsImpl.coq_Rmult RinvImpl.coq_Rinv
    req_Dec
...
let rowOpsInV_R =
  rowOpsInV RbaseSymbolsImpl.coq_Rplus RbaseSymbolsImpl.coq_Rmult
